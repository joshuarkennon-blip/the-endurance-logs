...
function LoadingSequence({ film, onComplete }) {
  ...
  return (
    <div className="flex flex-col justify-center items-start h-full p-8 gap-4">
      <div className="mb-2">
        <p className="text-[10px] tracking-[0.3em] uppercase mb-1" style={{ color: film.color }}>
          ENDURANCE // FILE SYSTEM SWAP
        </p>
        <p className="text-[22px] font-bold tracking-[0.15em]" style={{ color: film.glowColor, textShadow: `0 0 20px ${film.glowColor}` }}>
          {film.title}
        </p>
      </div>

      <div className="w-full space-y-1 font-mono text-[10px] text-console-muted">
        {lines.map((line, i) => (
      ...
      <div className="w-full mt-4">
        <div className="flex justify-between text-[8px] text-console-muted mb-1">
          <span>LOADING</span>
          <span>{progress}%</span>
        </div>
    ...
    <div className="flex flex-col h-full">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="text-[9px] tracking-[0.25em] uppercase" style={{ color: film.color }}>
            {film.transmission}
          </span>
        </div>
        <div className="flex items-center gap-4">
          <div className="flex gap-1">
            {film.themes.map(t => (
              <span
                key={t}
                className="text-[7px] px-1.5 py-0.5 border tracking-wider"
          ...
          <button onClick={onClose} className="console-btn text-[8px] px-2 py-1">
            EJECT ✕
          </button>
        </div>
      </div>
      ...
      {visibleSections >= 1 && (
        <div className="px-6 pt-5 pb-3 border-b border-console-border shrink-0 section-reveal">
          <div className="flex items-baseline gap-4">
            ...
            <span className="text-console-muted text-sm">{film.year}</span>
          </div>
          <p className="text-console-text text-[11px] mt-1 tracking-wider italic">
            "{film.tagline}"
          </p>
        </div>
      )}
      ...
      {visibleSections >= 2 && (
        <div className="px-6 pt-4 pb-3 border-b border-console-border shrink-0 section-reveal">
          <p className="text-[10px] text-console-muted tracking-widest uppercase mb-2">
            // TRANSMISSION OPENING
          </p>
          <p className="text-[13px] text-console-text leading-relaxed">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-6 py-4 space-y-6">
        {film.dossier.sections.map((section, i) => (
          visibleSections >= i + 3 && (
            <div key={i} className="section-reveal">
              ...
              <p className="text-[12px] text-console-text leading-relaxed pl-4">
                {section.body}
              </p>
            </div>
          )
        ...
        {visibleSections >= film.dossier.sections.length + 3 && (
          <div className="section-reveal pt-4 border-t border-console-border">
            <p className="text-[9px] text-console-muted tracking-widest">
      ...
      {!film ? (
        <div className="flex flex-col items-center justify-center h-full gap-4">
          <div className="text-center">
            <div className="text-5xl mb-4 pixel-art opacity-30">◈</div>
            <p className="text-[11px] tracking-[0.3em] text-console-muted uppercase">
              No Disc Loaded
            </p>
            <p className="text-[9px] text-console-muted mt-2 tracking-wider">
          ...
          <div className="absolute bottom-4 left-0 right-0 px-6">
            <div className="flex justify-between text-[8px] text-console-muted">
